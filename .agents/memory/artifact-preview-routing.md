...
**How to apply:** When cloning or importing an artifact, compare `.replit` workflow `PORT`/`waitForPort` with the artifact service metadata. Keep only the artifact-owned workflow for a service; a legacy workflow on the same port causes the managed workflow to fail with “Port ... is already in use.” If a failed restart leaves the old process alive, terminate only that artifact’s stale process tree before restarting. If the clone has no registered workflow, make a temporary identical copy of each `artifact.toml` and pass it through the platform’s validated artifact replacement flow, remove the temporary copy, then restart and verify the actual proxied URL.