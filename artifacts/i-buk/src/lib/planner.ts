...
const sessionMinutes = (session: Session) => session.durationMinutes > 0 ? session.durationMinutes : session.minutes;
...
export const topicSummaries = (workspace: Workspace): TopicStudySummary[] =>
  workspace.topics.map((topic) => {
    ...
    return {
      ...
      totalMinutes: sessions.reduce((total, session) => total + sessionMinutes(session), 0),
...
export const neglectedTopics = (workspace: Workspace, referenceDate = dateKey()): NeglectedTopic[] => {
  ...
  return workspace.topics
    ...
    .map((topic) => {
      ...
      const daysSinceStudied = summary.lastStudied
        ? Math.max(0, daysUntil(referenceDate, summary.lastStudied))
        : undefined;
      return {
        topic,
        course,
        summary,
        daysSinceStudied,
        reason: daysSinceStudied === undefined
          ? 'not studied yet'
          : daysSinceStudied === 0
            ? 'studied today'
    ...
    .filter((item): item is NeglectedTopic => item !== null)
    .sort((a, b) => {
      if (a.daysSinceStudied === undefined && b.daysSinceStudied !== undefined) return -1;
      if (a.daysSinceStudied !== undefined && b.daysSinceStudied === undefined) return 1;
      const byFreshness = (b.daysSinceStudied ?? Number.POSITIVE_INFINITY) - (a.daysSinceStudied ?? Number.POSITIVE_INFINITY);
      if (byFreshness !== 0) return byFreshness;
      if (a.summary.eventCount !== b.summary.eventCount) return a.summary.eventCount - b.summary.eventCount;
      if (a.topic.important !== b.topic.important) return a.topic.important ? -1 : 1;
...
export const recommendations = (workspace: Workspace, dayOffset: number, referenceDate = dateKey()): Recommendation[] => {
  ...
  workspace.sessions.forEach((session) => {
    if (daysUntil(sessionDate(session), referenceDate) >= -7) {
      const courseId = workspace.topics.find((topic) => topic.id === session.topicId)?.courseId ?? session.courseId;
      recentMinutesByCourse.set(courseId, (recentMinutesByCourse.get(courseId) ?? 0) + sessionMinutes(session));
    }
  });
  return workspace.topics
    .filter((topic) => topic.status !== 'done')
    .map((topic) => {
      ...
      const summary = summaries.get(topic.id);
      const exam = workspace.exams
        .filter((item) => item.courseId === topic.courseId && !item.complete)
        .sort((a, b) => daysUntil(a.date, referenceDate) - daysUntil(b.date, referenceDate))[0];
      ...
      const balance = Math.min(14, (recentMinutesByCourse.get(topic.courseId) ?? 0) / 45);
